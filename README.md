...
- Non-replay events have a 90-day TTL. Replay rows have a 30-day TTL. Animation RUM v2 aggregate tables separately retain 90 days from `captured_at`. Its terminal Postgres receipts become eligible for asynchronous cleanup 180 days after first receipt by default, while a quarantined canonical outbox envelope becomes eligible 7 days after quarantine; pending outbox work is never removed by retention cleanup.
- Animation RUM v2 retention is eventual rather than an exact deletion deadline. One replica runs a bounded cleanup cycle at startup and then on an interval, skips locked applications, removes target receipts before page receipts, and keeps any receipt that still has an outbox or child dependency.
...
| `INGEST_MODE`                                                                                                           | `kafka` (default in deploy) or `direct`; Animation RUM v2 drains its durable Postgres outbox through the selected transport     |
| `KAFKA_ENABLED`                                                                                                         | Master switch for Kafka producer. If false, the Animation RUM v2 outbox drains through ClickHouse even when `INGEST_MODE=kafka` |
| `ANIMATION_RUM_V2_OUTBOX_ENABLED`                                                                                       | Set to `false` only to pause Animation RUM v2 outbox draining; admission remains durable in Postgres                            |
| `ANIMATION_RUM_V2_OUTBOX_POLL_MS`, `ANIMATION_RUM_V2_OUTBOX_CONCURRENCY`, `ANIMATION_RUM_V2_OUTBOX_LEASE_MS`            | Outbox scheduling defaults: `1000` ms, `2` workers, and a `60000` ms lease                                                      |
| `ANIMATION_RUM_V2_OUTBOX_RETRY_BASE_MS`, `ANIMATION_RUM_V2_OUTBOX_RETRY_MAX_MS`, `ANIMATION_RUM_V2_OUTBOX_MAX_ATTEMPTS` | Durable retry defaults: `1000` ms exponential base, `300000` ms cap, and `288` attempts before quarantine                       |
| `ANIMATION_RUM_V2_RECEIPT_RETENTION_DAYS`                                                                               | Retention frozen into newly admitted receipts. Default `180`; must be an integer from `120` to `365`, otherwise startup fails   |
| `ANIMATION_RUM_V2_RETENTION_ENABLED`, `ANIMATION_RUM_V2_RETENTION_INTERVAL_MS`                                          | Retention worker switch and interval. Defaults: `true` and `3600000` ms; interval range `60000`–`86400000`                      |
| `ANIMATION_RUM_V2_QUARANTINE_ENVELOPE_RETENTION_DAYS`                                                                   | Days before a quarantined canonical envelope is eligible for cleanup. Default `7`; range `1`–`30`                               |
| `ANIMATION_RUM_V2_RETENTION_APP_LIMIT`, `ANIMATION_RUM_V2_RETENTION_BATCH_SIZE`                                         | Per-cycle application and per-application row bounds. Defaults `25` and `100`; ranges `1`–`100` and `10`–`500`                  |
| `ANIMATION_RUM_V2_RETENTION_MAX_ROWS_PER_CYCLE`, `ANIMATION_RUM_V2_RETENTION_MAX_CYCLE_MS`                              | Global row/time bounds per cycle. Defaults `1000` rows and `10000` ms; ranges `100`–`5000` and `1000`–`60000`                   |
...
| `KAFKA_FALLBACK_TO_CLICKHOUSE`                                                                                          | Legacy/non-v2 fallback; RUM v2 keeps retrying its durable outbox and never switches transport mid-message                       |
...
export function AnimatedCard() {
    ...
    createEffect(() =>
        condev.measureReactiveWork(() => {
            // Existing synchronous effect/computation work only.
        })
    )
...
This is still one Browser client and one `init()` call. `measureReactiveWork()` records only the caller-supplied synchronous callback as generic host `script` work and preserves its result or thrown error. Supported Solid versions (`>=1.9.10 <2`) have no public component-wide before/after commit or after-paint hook, and effect ordering is not a component timing boundary, so Condev never relabels this self-time as Solid render, update, component check, commit, DOM, paint, or GPU work. The package installs no automatic effect; wrap existing work only when that exact callback cost is useful. The `condevAnimationTarget` directive keeps the real Element and anonymous owner local, and unregisters when the Element owner is disposed even if the outer component remains alive. An explicitly authorized RUM v2 target can project only the closed `solid` framework value and `framework-adapter` capability, never component names, signals, props, state, text, selectors, classes, IDs, or URLs. Solid 2 prereleases are intentionally unsupported until their public lifecycle contract is verified.
