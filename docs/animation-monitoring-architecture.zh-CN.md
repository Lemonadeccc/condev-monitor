...
- 下一阶段继续补 Svelte/Solid 宿主 adapter、低侵入 Angular directive/compiler 集成、React/Vue/Angular 深层 owner/why-update、renderer 资源/传输与内部对象归因、heap/route soak、decode/upload/first-visible 等深层证据；
...
| Svelte / Solid               | 后续各自 framework package                                                     | 使用公开 dev/performance hook 或显式 marks，不修改框架私有对象                                                                                                                                                                           |
...
这些 adapter 只补框架 ownership 和渲染器内部信号。frame p95、INP、Long Task 等用户结果不会因为 React、Vue 或 Angular 而换一套定义；React render/commit、Vue update window、Angular check window 和 Three draw calls 也不能直接横向评分。
