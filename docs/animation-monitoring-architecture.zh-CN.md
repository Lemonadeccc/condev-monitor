...
| GSAP / Lenis / ScrollTrigger | core GSAP/ScrollTrigger inventory + 显式等价循环 analyzer + GSAP ticker observer；后续 motion package | 由宿主传入实例并只读 public API；已能提出有界 post-cleanup 增长候选并观察 ticker callback cadence，但不判 leak、不冒充页面 FPS；后续补 Lenis 与自动 checkpoint，不重复捆绑 GSAP                                                                  |
