...
- 下一阶段继续补 Solid 宿主 adapter、低侵入 Angular directive/compiler 集成、React/Vue/Angular/Svelte 深层 owner/why-update、renderer 资源/传输与内部对象归因、heap/route soak、decode/upload/first-visible 等深层证据；
...
| Angular                      | `packages/angular` + core component check-window                               | `@condev-monitor/angular/animation` 显式接 `ngDoCheck` → `ngAfterViewChecked` 组件检查窗口；`afterEveryRender({ read })` 仅做 application-wide post-render target 同步，不冒充 DOM update/render/commit/paint/GPU                        |
...
| Solid                        | 后续 framework package                                                         | 使用公开 owner/computation 能力或显式 marks，不修改框架私有对象                                                                                                                                                                          |
...
这些 adapter 只补框架 ownership 和渲染器内部信号。frame p95、INP、Long Task 等用户结果不会因为 React、Vue、Angular 或 Svelte 而换一套定义；React render/commit、Vue update window、Angular check window、Svelte tracked-dependency window 和 Three draw calls 也不能直接横向评分。
...
1. 完整 framework/motion adapters：React/Next owner/why-update/真实 commit、Vue/Angular/Svelte 深层 owner/why-update、Solid 更新归因、低侵入 Angular directive/compiler 集成，以及 GSAP/Lenis/ScrollTrigger ticker/自动 checkpoint/等价循环 analyzer；当前 React 只接显式 public Profiler，Vue 只接显式 public lifecycle update-window，Angular 只接显式 public component check-window 与 application-wide post-render target 同步，Svelte 只接显式 tracked-dependency `tick()` window，它们都只提供匿名目标归属；
