...
- 已实现本地 capture-window Resource Timing 聚合，以及 framework/renderer/lifecycle/work/media 五类有界 host evidence sink；已提供 React Profiler/manual commit、通用 renderer-host、Three `renderer.info`、GSAP/ScrollTrigger public API 和 Video RVFC/playback-quality 的无宿主依赖 helper；
...
SDK 驱动的每次 adapter 检查现在都会传入可选第二参数 `AnimationTargetAdapterInspectionContext`，其中 `evidenceWindow` 只含同一 collector 时钟域的 `startedAt`、`endedAt` 和 `selection-window`/`interaction-window`。registry provider 与 Browser `client.animation.registerTarget()` 回调都会收到它；参数保持可选，因此旧的一参 adapter/零参 provider 不会失效。新 renderer recorder 必须用这个窗口筛选已有样本并返回实际保留样本首尾，不能因 Overlay 重复 snapshot 而清空或重置数据。
...
| Three / R3F / WebGL / WebGPU   | core 通用 renderer-host + Three counter helper + `packages/animation-renderer` | 已接 public counters/context loss、可选 WebGL1/2 async GPU frame timer 与 WebGPU 单 pass timestamp query；后续补多 pass、薄引擎映射、targets/upload/readback          |
...
本轮已经补上其中的基础证据合同：target direct 的 Canvas 分轴尺寸/resize；连续交互的 input-to-visual、sample age、coalesced、progress/alignment、settle/overshoot 和 controller conflict；五类 page-level host evidence sink，以及 framework commit、通用 renderer-host、Three counter、GSAP lifecycle、Video RVFC helper。通用/Three/GSAP/Video helper 可以从显式传入的宿主 public evidence 读取闭集值；可选 renderer 包额外执行独占、稀疏、非阻塞的 WebGL1/2 GPU frame query，以及需宿主显式声明完整单 pass 边界、关联 submit 的 WebGPU timestamp query。它们仍不自动发现 framework owner、不协调 WebGPU 多 pass 或引擎私有 encoder、不做 heap/post-GC 泄漏判定，也不接管业务 cleanup。
...
2. 完整 renderer/media adapters：Three/R3F/Pixi/Babylon 的薄公开映射，WebGL/WebGPU 的多 pass/target/upload/readback/context/device 数据，Canvas2D 的 OffscreenCanvas Worker 时钟桥接、内部命中、异步编码/transfer 与完整业务资源归因，以及 media decode/upload/first-visible/visibility；当前 Canvas2D recorder 已接宿主显式完整逻辑帧、闭集 command、同步 readback/upload、backing/context 证据，通用 renderer-host/Three/Video helper 也已接公开状态，但引擎资源与生命周期语义仍不完整；
3. GPU query 后续：WebGL1/2 已有可选的异步、稀疏、非阻塞 frame timer，并验证 availability、disjoint、context loss、独占 ownership、队列和一次性消费；WebGPU 已有 feature-gated、单完整 pass、submit 后异步 map、device-loss 终态和乱序 sample 防回流的 timestamp timer；仍缺 WebGPU 多 pass/引擎私有 encoder 协调、跨重复 bundle 的强制仲裁、真实设备 capability/开销矩阵与引擎自带 profiler 协调；
