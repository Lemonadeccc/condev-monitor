...
- 下一阶段继续补 Solid 深层 computation/why-update、低侵入 Angular directive/compiler 集成、React/Vue/Angular/Svelte 深层 owner/why-update、renderer 资源/传输与内部对象归因、heap/route soak、decode/upload/first-visible 等深层证据；
...
1. 完整 framework/motion adapters：React/Next owner/why-update/真实 commit、Vue/Angular/Svelte 深层 owner/why-update、Solid 深层 computation/why-update、低侵入 Angular directive/compiler 集成，以及 GSAP/Lenis/ScrollTrigger ticker/自动 checkpoint/等价循环 analyzer；当前 React 只接显式 public Profiler，Vue 只接显式 public lifecycle update-window，Angular 只接显式 public component check-window 与 application-wide post-render target 同步，Svelte 只接显式 tracked-dependency `tick()` window，Solid 只接匿名 owner 与调用方显式同步 work self-time，它们都没有私有全树归因；
