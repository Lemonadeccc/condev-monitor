...
- 下一阶段继续补 Angular/Svelte/Solid 宿主 adapter、React/Vue 深层 owner/why-update、renderer 资源/传输与内部对象归因、heap/route soak、decode/upload/first-visible 等深层证据；
...
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| `recordFrameworkStats()` | framework、mount/update/hydrate phase、render/base-render、独立测得的 commit duration，或单独类型化的 lifecycle update window | 本地 framework work；不单独改变 `workAvoidance`  |
...
| Angular / Svelte / Solid     | 后续各自 framework package                                                     | 使用公开 dev/performance hook 或显式 marks，不修改框架私有对象                                                                                                                                                                           |
...
1. 完整 framework/motion adapters：React/Next owner/why-update/真实 commit、Vue 深层 owner/why-update、Angular/Svelte/Solid 自动更新归因，以及 GSAP/Lenis/ScrollTrigger ticker/自动 checkpoint/等价循环 analyzer；当前 React 只接显式 public Profiler，Vue 只接显式 public lifecycle update-window 与匿名目标归属，core helper 只接 manual/public inventory；
