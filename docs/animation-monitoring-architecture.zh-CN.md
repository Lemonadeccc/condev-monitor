...
- 已实现首版 Source Map 到 authored source 的本地离线候选定位；后续再开发 Chrome DevTools extension、逐帧 CPU/layer 专门视图、WebDriver BiDi/Selenium/Grid adapter 与真实设备矩阵。这些不会替代已经落地的页面浮层或 Labs。
...
本地 runner 永远先写本地产物目录。`trace.json.gz`、完整 Lighthouse JSON/HTML、selector、cookie、request/response body 与凭证属于 local-sensitive，不上传；平台只收不带 timeline、严格不超过 2 MiB 的 `animation-report`，以及事件数和紧凑 JSON 实际字节都受限（最多 4,000 条、4 MiB）的 `trace-index`。热身明细不上传；动作很多时按“高价值指标 × 动作顺序”公平裁剪每次尝试，先完成全量内存聚合再生成传输投影，避免后排动作饿死。Trace 索引优先保留动作 marker、已有动作归因和长耗时事件，同时收紧 stack 深度与 source 文本；原始 trace 不受这个派生视图代替。可选 `--source-map-manifest` 只读取本地、与 Scenario `release/dist` 精确匹配且位于 manifest 目录内的普通 Source Map v3，丢弃 `sourcesContent`，不抓取网络 map 或源码；平台仅得到脱敏 authored 候选与覆盖计数。这使 localhost:3000 能展示 Performance 栈/时间线、源码候选和 Lighthouse 摘要，但它不是 Chrome Performance panel 的完整替代品：原始 trace 仍应在本地 DevTools/Perfetto 打开，源码候选也不等同于根因。
...
本地报告可以保留有界帧尾、完整 coverage 和建议；Chromium Lab Trace 已支持显式本地 manifest 的 Source Map 候选定位，LoAF 自身的 authored-source 定位仍留在后续本地工具中实现。生产 RUM 只允许上传版本化 summary：
