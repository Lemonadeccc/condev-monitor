...
| GSAP / Lenis / ScrollTrigger | core public inventory + 显式等价循环 analyzer + GSAP ticker/Lenis scroll observer；后续 motion package | 由宿主传入实例并只读 public API；已能提出有界 post-cleanup 增长候选、观察 ticker callback cadence 与 Lenis 闭集 scroll state/progress/velocity sample，但不判 leak、不冒充页面 FPS 或 px/s；后续补 ScrollTrigger 深层状态与自动 checkpoint，不重复捆绑宿主库 |
