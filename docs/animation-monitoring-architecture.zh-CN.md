...
| Three / R3F / WebGL / WebGPU   | core 通用 renderer-host + Three counter helper + `packages/animation-renderer` | 已接 public counters/context loss、可选 WebGL1/2 async GPU frame timer 与 WebGPU 单 pass/同 command buffer 多 pass timestamp query；后续补跨 command buffer、薄引擎映射、targets/upload/readback |
...
本轮已经补上其中的基础证据合同：target direct 的 Canvas 分轴尺寸/resize；连续交互的 input-to-visual、sample age、coalesced、progress/alignment、settle/overshoot 和 controller conflict；五类 page-level host evidence sink，以及 framework commit、通用 renderer-host、Three counter、GSAP lifecycle、Video RVFC helper。通用/Three/GSAP/Video helper 可以从显式传入的宿主 public evidence 读取闭集值；可选 renderer 包额外执行独占、稀疏、非阻塞的 WebGL1/2 GPU frame query，以及需宿主显式声明完整单 pass，或同一个 command buffer 首/末 pass 边界与关联 submit 的 WebGPU timestamp query。它们仍不自动发现 framework owner、不协调 WebGPU 跨 command buffer/submit 或引擎私有 encoder、不做 heap/post-GC 泄漏判定，也不接管业务 cleanup。
...
2. 完整 renderer/media adapters：Three/R3F/Pixi/Babylon 的薄公开映射，WebGL/WebGPU 的跨 command buffer/target/upload/readback/context/device 数据，Canvas2D 的 OffscreenCanvas Worker 时钟桥接、内部命中、异步编码/transfer 与完整业务资源归因，以及 media decode/upload/first-visible/visibility；当前 Canvas2D recorder 已接宿主显式完整逻辑帧、闭集 command、同步 readback/upload、backing/context 证据，通用 renderer-host/Three/Video helper 也已接公开状态，但引擎资源与生命周期语义仍不完整；
3. GPU query 后续：WebGL1/2 已有可选的异步、稀疏、非阻塞 frame timer，并验证 availability、disjoint、context loss、独占 ownership、队列和一次性消费；WebGPU 已有 feature-gated 单完整 pass、同一 command buffer 首/末 pass 区间、submit 后异步 map、device-loss 终态和乱序 sample 防回流的 timestamp timer；仍缺 WebGPU 跨 command buffer/submit 与引擎私有 encoder 协调、跨重复 bundle 的强制仲裁、真实设备 capability/开销矩阵与引擎自带 profiler 协调；
