...
import { type ChromeDebuggingEndpointOptions, waitForChromeDebuggingEndpoint } from './chrome-debugging-endpoint'
...
export function waitForLighthouseChromeEndpoint(port: number, options: ChromeDebuggingEndpointOptions = {}): Promise<void> {
    return waitForChromeDebuggingEndpoint(port, { ...options, description: 'Lighthouse Chrome' }).then(() => undefined)
}
