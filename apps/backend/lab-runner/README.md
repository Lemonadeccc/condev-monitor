...
The run-scoped video dropped-frame metric reads the current DOM's `getVideoPlaybackQuality()` counters when the probe stops. It validates every readable counter pair, divides summed `droppedVideoFrames` by summed `totalVideoFrames`, and uses the valid total frames as `samples`. Complete, partial-surface, no-video, zero-total-frame, read/validation-error, and unsupported states remain distinct. The denominator is the Playback Quality total (displayed plus dropped), not decoded-only or presented-only frames. This cumulative final snapshot is not a measurement-window delta: removed elements and counter resets are not reconstructed, and the metric is intentionally absent from action scope. It provides no decode, GPU-upload, first-visible, stall, or per-frame presentation evidence.
