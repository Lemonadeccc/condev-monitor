...
type ProjectionCandidate = { captureId: string; eventId: string; routeKey: string; capturedAt: string }
...
export const ANIMATION_RUM_V3_PIPELINE_CLOCK = Symbol('ANIMATION_RUM_V3_PIPELINE_CLOCK')

export const ANIMATION_RUM_V3_PIPELINE_LIMITS = Object.freeze({
...
const DATABASE_ROW_LIMIT = ANIMATION_RUM_V3_PIPELINE_LIMITS.comparisonLimit + 1
const CLICKHOUSE_CAPTURE_CHILD_LIMITS = Object.freeze({ metrics: 3, providerEvidence: 1 })
const CLICKHOUSE_EXPECTED_ROWS_TO_READ =
    ANIMATION_RUM_V3_PIPELINE_LIMITS.comparisonLimit *
...
const CLICKHOUSE_READ_SETTINGS = Object.freeze({
    ...
    max_result_rows: String(ANIMATION_RUM_V3_PIPELINE_LIMITS.comparisonLimit),
...
function boundedCount(value: unknown): BoundedCount & { valid: boolean } {
    ...
    return {
        count: Math.min(parsed, ANIMATION_RUM_V3_PIPELINE_LIMITS.comparisonLimit),
        truncated: parsed > ANIMATION_RUM_V3_PIPELINE_LIMITS.comparisonLimit,
        valid: true,
    }
}
...
@Injectable()
export class AnimationRumV3SoftNavigationPipelineService {
    ...
    constructor(
        ...
        @Optional()
        @Inject(ANIMATION_RUM_V3_PIPELINE_CLOCK)
        readClock?: () => Date
    ) {
    ...
    async read(userId: number, appId: string) {
        ...
        if (!(observedAtDate instanceof Date) || !Number.isFinite(observedAtDate.getTime())) {
            throw new Error('Animation RUM v3 soft-navigation pipeline clock returned an invalid date')
        }
        const observedAt = observedAtDate.toISOString()
        const from = new Date(observedAtDate.getTime() - ANIMATION_RUM_V3_PIPELINE_LIMITS.lookbackSeconds * 1_000).toISOString()
        const projectionCutoff = new Date(
            observedAtDate.getTime() - ANIMATION_RUM_V3_PIPELINE_LIMITS.projectionGraceSeconds * 1_000
        ).getTime()
        ...
        const receiptTruncated = receiptRows.length > ANIMATION_RUM_V3_PIPELINE_LIMITS.comparisonLimit
        const retainedReceipts = receiptRows.slice(0, ANIMATION_RUM_V3_PIPELINE_LIMITS.comparisonLimit)
        ...
        for (const row of retainedReceipts) {
            ...
            const routeKey = isAnimationRumV2RouteKey(row.routeKey) ? row.routeKey : null
            const capturedAt = isoTimestamp(row.capturedAt)
            if (!transitionedAt || !captureId || !eventId || !routeKey || !capturedAt) {
                statePairMismatch += 1
                continue
            }
            if (Date.parse(transitionedAt) <= projectionCutoff) candidates.push({ captureId, eventId, routeKey, capturedAt })
        }
        ...
        const boundedStatePairMismatch = Math.min(statePairMismatch, ANIMATION_RUM_V3_PIPELINE_LIMITS.comparisonLimit)
        ...
        return {
            ...
            rumContractVersion: 3 as const,
            observedAt,
            status,
            window: { ...ANIMATION_RUM_V3_PIPELINE_LIMITS },
            receipts: {
        ...
        return this.dataSource.query<ReceiptActivityRow[]>(
            `
                SELECT receipt.capture_id AS "captureId",
                       ...
                       receipt.captured_at AS "capturedAt",
                ...
                LEFT JOIN public.animation_rum_v3_soft_navigation_outbox AS outbox
            ...
            `
                WITH bounded AS MATERIALIZED (
                    SELECT outbox.created_at,
                           ...
                           outbox.lease_until
                    FROM public.animation_rum_v3_soft_navigation_outbox AS outbox
                    INNER JOIN public.application AS application
    ...
    private outboxAggregate(row: OutboxAggregateRow | undefined): OutboxAggregate {
        ...
        const boundedDue = Math.min(due ?? 0, ANIMATION_RUM_V3_PIPELINE_LIMITS.comparisonLimit)
        const boundedRetrying = Math.min(retrying ?? 0, ANIMATION_RUM_V3_PIPELINE_LIMITS.comparisonLimit)
        const boundedLeased = Math.min(leased ?? 0, ANIMATION_RUM_V3_PIPELINE_LIMITS.comparisonLimit)
        const valid =
            count.valid &&
            due !== null &&
            retrying !== null &&
    ...
    private async readProjection(appId: string, candidates: ProjectionCandidate[]): Promise<ProjectionView> {
        ...
        const routeKeys = [...new Set(candidates.map(candidate => candidate.routeKey))]
        const capturedAt = candidates.map(candidate => candidate.capturedAt).sort()
        const capturedFrom = clickhouseTime(capturedAt[0]!)
        const capturedTo = clickhouseTime(capturedAt[capturedAt.length - 1]!)
        try {
            const result = await this.clickhouse.query({
                query: `
                    WITH ${createAnimationRumV3ProjectionSql({
                    ...
                    ORDER BY capture_id
                    LIMIT ${ANIMATION_RUM_V3_PIPELINE_LIMITS.comparisonLimit}
                `,
                query_params: {
                    appId,
                    routeKeys,
            ...
            for (const row of json.data) {
                ...
                matched += 1
                const scope = row.scope === 'page' ? 'page' : null
    ...
    private status(input: {
        ...
        const stalePending =
            ...
            input.observedAt - oldestPendingAt > ANIMATION_RUM_V3_PIPELINE_LIMITS.outboxDelaySeconds * 1_000
