...
import { ANIMATION_RUM_V3_PIPELINE_LIMITS, AnimationRumV3SoftNavigationPipelineService } from './animation-rum-v3-pipeline.service'
...
const publishedReceipt = (overrides: Partial<ReceiptFixture> = {}): ReceiptFixture => ({
    ...
    capturedAt: '2026-08-27T11:54:00.000Z',
    ...overrides,
})
...
const projectionRow = (overrides: Partial<ProjectionFixture> = {}): ProjectionFixture => ({
    ...
    projection_observed_metric_count: 3,
    projection_matching_metric_count: 3,
...
function createService(
    ...
    const dataSource = {
        query: jest.fn(async (sql: string, params?: unknown[]) => {
            ...
            if (sql.includes('animation_rum_v3_soft_navigation_capture_receipt AS receipt')) return receipts
    ...
    const service = new AnimationRumV3SoftNavigationPipelineService(
        dataSource as any,
        clickhouse as any,
        { get: () => 'lemonade' } as any,
        applications as any,
...
describe('AnimationRumV3SoftNavigationPipelineService', () => {
    it('authorizes before bounded store reads and reports Kafka ACK separately from the ClickHouse completion marker', async () => {
        ...
        expect(
            dataSource.query.mock.calls.every(call => call[0].includes(`LIMIT ${ANIMATION_RUM_V3_PIPELINE_LIMITS.comparisonLimit + 1}`))
        ).toBe(true)
        ...
        expect(query.query).toContain('animation_rum_soft_navigation_captures_v3 FINAL')
        expect(query.query).toContain('animation_rum_soft_navigation_metrics_v3 FINAL')
        expect(query.query).toContain('animation_rum_soft_navigation_provider_evidence_v3 FINAL')
        ...
        expect(query.query).toContain(`LIMIT ${ANIMATION_RUM_V3_PIPELINE_LIMITS.comparisonLimit}`)
        expect(query.query_params).toEqual({
            appId: APP_ID,
            routeKeys: ['catalog.detail'],
            capturedFrom: '2026-08-27 11:54:00.000',
            capturedTo: '2026-08-27 11:54:00.000',
            captureIds: ['capture_12345678'],
        })
        expect(query.query).toContain('(app_id, route_key, capture_id) IN')
        expect(query.query).toContain('captured_at >= (SELECT min(captured_at)')
        expect(query.clickhouse_settings).toEqual(
            expect.objectContaining({ max_result_rows: '500', max_rows_to_read: '5000', max_memory_usage: '67108864', max_threads: 2 })
        )
