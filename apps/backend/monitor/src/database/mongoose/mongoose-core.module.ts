...
export class MongooseCoreModule implements OnApplicationShutdown {
        ...
        const connectionProvider = {
            ...
            useFactory: async (mongooseModuleOptions: MongooseModuleFactoryOptions): Promise<any> => {
                ...
                return await lastValueFrom(
                    defer(async () =>
                        mongooseConnectionFactory(
                            await this.createMongooseConnection(uri as string, mongooseOptions, { lazyConnection, onConnectionCreate }),
                            mongooseConnectionName
                        )
                    ).pipe(
                        handleRetry(retryAttempts, retryDelay, verboseRetryLog),
    ...
    private static createAsyncProviders(options: MongooseModuleAsyncOptions): Provider[] {
    ...
    private static createAsyncOptionsProvider(options: MongooseModuleAsyncOptions): Provider {
        ...
        const inject = [(options.useClass || options.useExisting) as Type<MongooseOptionsFactory>]
        return {
            provide: MONGOOSE_MODULE_OPTIONS,
            useFactory: async (optionsFactory: MongooseOptionsFactory) => await optionsFactory.createMongooseOptions(),
            inject,
        }
    }
