...
// 获取控制点位置
function getControlPoints(startPoint: { x: number; y: number }, currentPoint: { x: number; y: number }) {
    const rect = {
...
function getControlPointAt(
    ...
    for (const [name, point] of Object.entries(controlPoints)) {
        if (Math.abs(x - point.x) <= threshold && Math.abs(y - point.y) <= threshold) {
            return name
        }
    }
    return null
...
> {
    try {
        ...
        const image = gui.Image.createFromBuffer((await (await screenshotPNG).getBuffer('image/png')).buffer, display.scaleFactor)
        container.onMouseLeave = (container: gui.Container, event: gui.MouseEvent) => {
            if (!isMonitorSelected) {
                win.close()
            }
        }
        container.onDraw = (container: gui.Container, painter: gui.Painter, dirty: any) => {
            painter.clear()
        ...
        container.onMouseDown = (container: any, event: gui.MouseEvent) => {
            ...
            if (isRectComplete) {
                ...
                // 检查是否点击了控制点
                const controlPoint = getControlPointAt(mouseX, mouseY, startPoint, currentPoint, isRectComplete)
                if (controlPoint) {
        ...
        container.onMouseMove = (container: gui.Container, event: gui.MouseEvent) => {
            const mouseX = event.positionInView.x
            const mouseY = event.positionInView.y
            ...
            if (isRectComplete) {
                isHoveringButton = isPointInConfirmButton(mouseX, mouseY, startPoint, currentPoint)
            ...
            if (isDraggingControl && activeControlPoint) {
                ...
                switch (activeControlPoint) {
                    ...
                    case 'bottom':
                        // 下边控制点：只能垂直拖动，调整下边位置
                        currentPoint.y = Math.min(display.bounds.height, Math.max(mouseY, startPoint.y + 30))
                        break
                    case 'left':
                        // 左边控制点：只能水平拖动，调整左边位置
                    ...
                    case 'right':
                        // 右边控制点：只能水平拖动，调整右边位置
                        currentPoint.x = Math.min(display.bounds.width, Math.max(mouseX, startPoint.x + 30))
                        break
                }
                container.schedulePaint()
            } else if (isDragging) {
        ...
        let rect: { x: number; y: number; width: number; height: number } = undefined as any
        if (isRectComplete) {
            rect = {
                x: startPoint.x,
                y: startPoint.y,
            ...
            if (saveFileFullPath?.length) {
                ...
                const img = await screenImage.crop(rect.x, rect.y, rect.width, rect.height)
