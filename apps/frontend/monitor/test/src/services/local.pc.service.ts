import { clipboard as nutClipboard, keyboard as nutKeyboard, mouse as nutMouse } from '@nut-tree-fork/nut-js'
import { Monitor, Window } from 'node-screenshots'

import { AbstractMonitor, AbstractWindow, IPCService, PNGBuffer } from '../interfaces/pc.service.interface.js'
...
export const localPCService: IPCService = {
    ...
    allMonitors: () => Promise.resolve(Monitor.all().map(monitor => new LocalMonitor(monitor))),
    allWindows: () => Promise.resolve(Window.all().map(window => new LocalWindow(window))),
