import { AbstractMonitor, AbstractWindow, IPCService, Point, Rect } from '../interfaces/pc.service.interface.js'
...
async function postJSON<T>(endpoint: string, path: string, body: any): Promise<T> {
    const res = await fetch(url(endpoint, path), {
        method: 'POST',
...
export function createRemotePCService(endpoint: string): IPCService {
    ...
    return {
        ...
        async screenShot(saveFileFullPath?: string) {
            const data = await postJSON<{ rect: Rect; monitor: any | null } | undefined>(base, '/screenshot', { saveFileFullPath })
            if (!data) return undefined
            const monitor = data.monitor ? new RemoteMonitor(base, data.monitor) : null
