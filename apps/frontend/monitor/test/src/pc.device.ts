...
export default class PCDevice implements AbstractInterface {
    ...
    private async findWindow(): Promise<WindowInfo> {
        ...
        const targetWindow = windows.find(w => {
            ...
            const titleMatch =
                !this.options.launchOptions?.windowInfo?.title || w.title.includes(this.options.launchOptions?.windowInfo?.title)
            const appNameMatch =
                !this.options.launchOptions?.windowInfo?.appName || w.appName.includes(this.options.launchOptions?.windowInfo?.appName)
        ...
        if (
            ...
            if (!targetMonitor) {
                ...
                if (!targetMonitor && this.options.launchOptions?.screenArea?.moniterIdx !== undefined) {
                    targetMonitor =
                        allMonitors[Math.min(Math.max(this.options.launchOptions?.screenArea?.moniterIdx ?? 0, 0), allMonitors.length - 1)]
                } else {
        ...
        return [
            ...
            defineActionSwipe(async (param: ActionSwipeParam) => {
                ...
                if (element?.center && param.end?.center) {
                    ...
                    console.warn(
                        `postion ${param.start?.center ? 'start not found' : ''} ${param.end?.center ? 'end not found' : ''}, skip swipe`
                    )
                }
            }),
    ...
    public async screenshotBase64(): Promise<string> {
        const targetInfo = await this.targetFinder()
        const screenshot = await targetInfo.captureImage()
