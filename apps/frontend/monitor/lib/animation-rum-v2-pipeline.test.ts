...
import {
    AnimationRumV2PipelineApiError,
    animationRumV2PipelineStatusMeta,
    formatAnimationRumV2PipelineCount,
    getAnimationRumV2Pipeline,
} from './animation-rum-v2-pipeline'

type FetchCall = { input: RequestInfo | URL; init?: RequestInit }

const diagnostic: AnimationRumV2PipelineDiagnostic = {
...
describe('Animation RUM v2 pipeline client', () => {
    it('uses a same-origin bounded GET and rebuilds only the closed response fields', async () => {
        ...
        await withFetchStub(
            ...
            async calls => {
                const result = await getAnimationRumV2Pipeline('app /?&=中文', controller.signal)
                assert.deepEqual(result, diagnostic)
                assert.equal('captureId' in result, false)
                assert.equal('error' in result, false)
                assert.equal(calls.length, 1)
                assert.equal(calls[0].input, '/api/animation/rum-v2/pipeline?appId=app+%2F%3F%26%3D%E4%B8%AD%E6%96%87')
    ...
    it('surfaces throttling without exposing arbitrary backend error text', async () => {
        await withFetchStub(
            ...
            async () => {
                await assert.rejects(getAnimationRumV2Pipeline('application-1'), error => {
                    assert.ok(error instanceof AnimationRumV2PipelineApiError)
    ...
    it('accepts an explicit receipt state mismatch without pretending every row has a valid closed state', async () => {
        ...
        await withFetchStub(
            ...
            async () => assert.deepEqual(await getAnimationRumV2Pipeline('application-1'), inconsistent)
        )
    })
    ...
    it('rejects invalid JSON, invalid envelopes, contradictory totals, and nullable projection mismatches', async () => {
        ...
        for (const invalid of invalidBodies) {
            await withFetchStub(
                ...
                async () => {
                    await assert.rejects(getAnimationRumV2Pipeline('application-1'), {
                        name: 'AnimationRumV2PipelineApiError',
    ...
    it('keeps every status label closed and formats truncated counts explicitly', () => {
        const statuses: AnimationRumV2PipelineStatus[] = [
            'idle',
            'in-flight',
            'healthy',
            'delayed',
