...
For an attached run, Runner contract v6 makes the authenticated platform claim authoritative for the complete measurement contract as well as the browser and execution envelope. Negotiation and claim both return the required metric-catalog and budget capabilities; the Runner requires those capabilities, the claim config, and the negotiated contract to match before browser navigation. It sends contract v6 during negotiation, claim, progress updates, and artifact uploads, replaces the local Scenario measurement contract before validation and execution, and retains the claimed value in the report. A current Monitor also accepts Runner contracts v4 and v5: v4 is limited to metric catalog/budget v3 and omits capability fields, while v5 carries those fields and can run catalog/budget v4 but accepts only the original eight report action kinds. Contract v6 adds `touch-tap`, `touch-swipe`, `touch-pinch`, and `pen-path` to the formal report action enum. The Monitor rejects incompatible catalog or action evidence before permanent artifact storage and accepts a completed report only when every measurement-contract field exactly matches the stored run configuration.
