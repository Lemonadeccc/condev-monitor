...
## Reviewed animation coverage

`AnimationCoverageManifestV1` is a local-only inventory for one static `routeKey` and one exact local Scenario SHA-256. Every item uses closed kinds and safe tokens. Critical items must declare an outcome contract, and `renderer-object` items must declare an adapter-backed hit contract. Draft or needs-review manifests cannot be projected.

`projectAnimationCoverageManifestV1()` accepts the reviewed manifest plus exactly one result for every declared item. It emits only `LabAnimationCoverageV1`: a manifest digest, the matched-review marker, closed item identity/status/reason tokens, and totals derived from the complete reviewed inventory. `critical` marks a subset that requires an outcome contract; it does not filter the inventory or change the totals. Critical and non-critical items both contribute to `declared`, `discovered`, `executed`, `passed`, and `uncovered`. The local route key, Scenario digest, outcome keys, renderer object keys, selectors, URLs, and business data are not retained. The validator enforces `passed <= executed <= declared` and `uncovered = declared - passed` against the actual closed item set.

Semantics v3 adds this upload-safe `coverage` projection. `ANIMATION_LAB_CURRENT_SEMANTICS_VERSION` identifies v3 for newly emitted reports. The deprecated compatibility constant `ANIMATION_LAB_SEMANTICS_VERSION` remains 2 so existing callers do not accidentally change their wire contract; explicit `ANIMATION_LAB_SEMANTICS_V2_VERSION` and `ANIMATION_LAB_SEMANTICS_V3_VERSION` constants are also exported. `AnimationLabSemanticsV2` and `validateAnimationLabSemanticsV2()` remain available for stored v2 reports. `LabMeasurementContractV2.contractVersion` remains 2.
...
For an attached run, Runner contract v12 makes the authenticated platform claim authoritative for the complete measurement contract, browser/execution envelope, and declared authentication mode. Negotiation, claim, progress updates, and artifact uploads all require v12; the Monitor currently rejects every older or future Runner contract. v11 introduced the closed target/driver/auth/cross-origin/power/thermal execution-provenance tuple, while v12 binds `authenticationMode` (`none` or local Playwright storage state) to the claimed configuration so the Runner cannot silently execute an authenticated Scenario for a run declared unauthenticated, or the reverse. The local reviewed Scenario remains authoritative for actions, selectors, semantic identities, completion gates, and optional critical-animation coverage. The Monitor accepts a completed report only when measurement, execution provenance, authentication mode, semantics, and coverage projection satisfy their exact closed contracts.
