...
test('target builder requires an explicit semantic target key and keeps direct, overlap, and adapter relations separate', () => {
    ...
    const vueTarget = targetSnapshot()
    vueTarget.inventory.uiFrameworks = ['vue']
    vueTarget.owners = [{ relation: 'framework-owner', framework: 'vue' }]
    const vueReport = toAnimationRumV2TargetReport(
        snapshot,
        vueTarget,
        projectionOptions(runtime, {
            eventId: 'event_target_vue_12345678',
            captureId: 'capture_target_vue_12345678',
            targetKey: 'hero-vue',
        })
    )
    assert.equal(vueReport.context.runtime.framework, 'vue')
    ...
    const svelteTarget = targetSnapshot()
    svelteTarget.inventory.uiFrameworks = ['svelte']
    svelteTarget.owners = [{ relation: 'framework-owner', framework: 'svelte' }]
    const svelteReport = toAnimationRumV2TargetReport(
        snapshot,
        svelteTarget,
        projectionOptions(runtime, {
            eventId: 'event_target_svelte_12345678',
            captureId: 'capture_target_svelte_12345678',
            targetKey: 'hero-svelte',
        })
    )
    assert.equal(svelteReport.context.runtime.framework, 'svelte')
