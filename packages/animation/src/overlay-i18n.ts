...
const EN_MESSAGES = {
    ...
    localSemanticEvidence: 'Local semantic evidence',
    localEvidenceProviderSummary: '{providers} providers · {dropped} dropped · {rejected} rejected',
    localEvidenceBoundary:
        'Separate from measured browser evidence: media stages are caller-attested and motion checkpoints are explicit temporal correlations. They do not prove browser decode, GPU upload, presentation, visual completion, or causality and are never uploaded by this overlay.',
    localEvidenceMediaHeading: 'Caller-attested media stages · {retained} retained · {dropped} dropped',
    localEvidenceMotionHeading: 'Explicit motion checkpoint correlations · {retained} retained · {dropped} dropped',
    localEvidenceNoMedia: 'No caller-attested media stage retained.',
    localEvidenceNoMotion: 'No explicit motion checkpoint correlation retained.',
    localEvidenceUnavailable: 'Local semantic evidence is unavailable because its provider returned invalid data.',
    localEvidenceMediaAttemptSummary: '{kind} · {outcome} · {duration}',
    localEvidenceMotionInteractionSummary: '{kind} · {outcome} · {duration}',
    localEvidenceDecodeReady: 'decode ready',
    localEvidenceUploadReady: 'upload ready',
    localEvidenceFirstVisible: 'first visible',
    localEvidenceMediaStageSummary: '{stage} +{elapsed} · declared {duration} · {bytes} · {items} items',
    localEvidenceNoDeclaredStage: 'No declared stage in this retained attempt.',
    localEvidenceMotionCheckpointSummary:
        'before {before} {beforeMeasured}/{beforeConfigured} → after {after} {afterMeasured}/{afterConfigured}',
    localEvidenceMotionSourceSummary: 'after: GSAP {gsap} · Lenis {lenis} · ScrollTrigger {scrollTrigger}',
    automaticPageEvidence: 'Automatic page evidence',
    pageEvidenceSummary: '{samples} samples · {scopes} document scopes',
    pageEvidenceBoundary:
        'Local aggregate browser evidence only. Hidden/offscreen and reduced-motion values are review candidates—not proof of avoidable work, causality, policy violations, renderer ownership, or GPU duration. No selector, text, URL, animation name, keyframe, media source, coordinate, or raw event is rendered or uploaded by this panel.',
    pageEvidenceNotConnected: 'This source does not provide automatic page evidence.',
    pageEvidenceUnavailable: 'Automatic page evidence is unavailable because its provider returned invalid data.',
    pageEvidenceDisabled: 'Automatic page evidence is disabled for this client.',
    pageEvidenceScopesTruncated: 'Document-scope discovery reached its local bound.',
    pageEvidenceAnimationsHeading: 'CSS / Web Animations · {status}',
    pageEvidenceAnimationSummary: 'current {total} · running {running} · infinite {infinite} · peak {peak} · dropped {dropped}',
    pageEvidenceLifecycleSummary:
        'animation start/end/cancel {start}/{end}/{cancel} · transition run/end/cancel {run}/{transitionEnd}/{transitionCancel}',
    pageEvidenceMediaHeading: 'Video cadence · {status}',
    pageEvidenceMediaSummary:
        'current {current} · playing {playing} · probes {probes} · rVFC {supported}/{unsupported} · dropped {dropped}',
    pageEvidenceRendererHeading: 'Renderer surfaces · {status}',
    pageEvidenceRendererSummary: 'SVG {svg} · Canvas? {unknown} · 2D {canvas2d} · WebGL {webgl} · WebGL2 {webgl2} · WebGPU {webgpu}',
    pageEvidenceRendererContextSummary: 'context observations {contexts} · lost/restored {lost}/{restored} · GPU timing {gpu}',
    pageEvidenceWorkHeading: 'Visibility / offscreen candidates · {status}',
    pageEvidenceWorkSummary:
        'now hidden animation/video {hiddenAnimations}/{hiddenVideos} · offscreen {offscreenAnimations}/{offscreenVideos}',
    pageEvidenceWorkCandidateSummary:
        'candidate signals hidden animation/video {hiddenAnimations}/{hiddenVideos} · offscreen animation/video {offscreenAnimations}/{offscreenVideos} · work duration {duration}',
    pageEvidenceReducedHeading: 'Reduced-motion candidates · {status}',
    pageEvidenceReducedSummary:
        'preference {preference} · running {running} · infinite {infinite} · video {videos} · review samples {samples}',
    pageEvidenceStatusMeasured: 'measured',
    pageEvidenceStatusPartial: 'partial',
    pageEvidenceStatusNotObserved: 'not observed',
    pageEvidenceStatusNotApplicable: 'not applicable',
    pageEvidenceStatusUnsupported: 'unsupported',
    pageEvidenceStatusUnknown: 'unknown',
    pageEvidencePreferenceReduce: 'reduce',
    pageEvidencePreferenceNoPreference: 'no preference',
    pageEvidencePreferenceUnknown: 'unknown',
    refreshAtLeast: 'Refresh ≥ {seconds}s',
    overview: 'Overview',
    pageEvidenceView: 'Page',
    ...
    frameworkUpdateCauses: 'update causes {causes} · observed {count}',
    frameworkUpdateWindowP95: 'Framework update window p95',
    frameworkCheckWindowP95: 'Framework check window p95',
...
const ZH_MESSAGES = {
    ...
    localSemanticEvidence: '本地语义证据',
    localEvidenceProviderSummary: '{providers} 个提供者 · 丢弃 {dropped} · 拒绝 {rejected}',
    localEvidenceBoundary:
        '与浏览器实测证据明确分开：媒体阶段由调用方声明，动效检查点只是显式的时间关联；它们不能证明浏览器解码、GPU 上传、屏幕呈现、视觉完成或因果关系，浮窗也不会上报这些记录。',
    localEvidenceMediaHeading: '调用方声明的媒体阶段 · 保留 {retained} · 丢弃 {dropped}',
    localEvidenceMotionHeading: '显式动效检查点关联 · 保留 {retained} · 丢弃 {dropped}',
    localEvidenceNoMedia: '尚未保留调用方声明的媒体阶段。',
    localEvidenceNoMotion: '尚未保留显式动效检查点关联。',
    localEvidenceUnavailable: '本地语义证据不可用：提供者返回了无效数据。',
    localEvidenceMediaAttemptSummary: '{kind} · {outcome} · {duration}',
    localEvidenceMotionInteractionSummary: '{kind} · {outcome} · {duration}',
    localEvidenceDecodeReady: '解码就绪',
    localEvidenceUploadReady: '上传就绪',
    localEvidenceFirstVisible: '首次可见',
    localEvidenceMediaStageSummary: '{stage} +{elapsed} · 声明耗时 {duration} · {bytes} · {items} 项',
    localEvidenceNoDeclaredStage: '这条保留记录没有声明阶段。',
    localEvidenceMotionCheckpointSummary:
        '之前 {before} {beforeMeasured}/{beforeConfigured} → 之后 {after} {afterMeasured}/{afterConfigured}',
    localEvidenceMotionSourceSummary: '之后：GSAP {gsap} · Lenis {lenis} · ScrollTrigger {scrollTrigger}',
    automaticPageEvidence: '自动页面证据',
    pageEvidenceSummary: '{samples} 次采样 · {scopes} 个文档作用域',
    pageEvidenceBoundary:
        '这里只显示浏览器本地聚合证据。后台/离屏和减少动态效果数值只是待复核候选，不能证明存在可避免工作、因果关系、策略违规、渲染器归属或 GPU 耗时。此面板不会渲染或上报 selector、文本、URL、动画名称、关键帧、媒体地址、坐标或原始事件。',
    pageEvidenceNotConnected: '当前数据源没有提供自动页面证据。',
    pageEvidenceUnavailable: '自动页面证据不可用：提供者返回了无效数据。',
    pageEvidenceDisabled: '当前客户端已关闭自动页面证据。',
    pageEvidenceScopesTruncated: '文档作用域发现数量达到本地上限。',
    pageEvidenceAnimationsHeading: 'CSS / Web Animations · {status}',
    pageEvidenceAnimationSummary: '当前 {total} · 运行中 {running} · 无限循环 {infinite} · 峰值 {peak} · 丢弃 {dropped}',
    pageEvidenceLifecycleSummary: '动画开始/结束/取消 {start}/{end}/{cancel} · 过渡运行/结束/取消 {run}/{transitionEnd}/{transitionCancel}',
    pageEvidenceMediaHeading: '视频帧节奏 · {status}',
    pageEvidenceMediaSummary: '当前 {current} · 播放中 {playing} · 探针 {probes} · rVFC {supported}/{unsupported} · 丢弃 {dropped}',
    pageEvidenceRendererHeading: '渲染器表面 · {status}',
    pageEvidenceRendererSummary: 'SVG {svg} · Canvas 未知 {unknown} · 2D {canvas2d} · WebGL {webgl} · WebGL2 {webgl2} · WebGPU {webgpu}',
    pageEvidenceRendererContextSummary: '上下文观测 {contexts} · 丢失/恢复 {lost}/{restored} · GPU 计时 {gpu}',
    pageEvidenceWorkHeading: '后台 / 离屏候选 · {status}',
    pageEvidenceWorkSummary: '当前后台动画/视频 {hiddenAnimations}/{hiddenVideos} · 离屏 {offscreenAnimations}/{offscreenVideos}',
    pageEvidenceWorkCandidateSummary:
        '候选信号：后台动画/视频 {hiddenAnimations}/{hiddenVideos} · 离屏动画/视频 {offscreenAnimations}/{offscreenVideos} · 工作耗时 {duration}',
    pageEvidenceReducedHeading: '减少动态效果候选 · {status}',
    pageEvidenceReducedSummary: '偏好 {preference} · 运行中 {running} · 无限循环 {infinite} · 视频 {videos} · 待复核样本 {samples}',
    pageEvidenceStatusMeasured: '已测量',
    pageEvidenceStatusPartial: '部分证据',
    pageEvidenceStatusNotObserved: '未观测',
    pageEvidenceStatusNotApplicable: '不适用',
    pageEvidenceStatusUnsupported: '不支持',
    pageEvidenceStatusUnknown: '未知',
    pageEvidencePreferenceReduce: '减少动态效果',
    pageEvidencePreferenceNoPreference: '无减少偏好',
    pageEvidencePreferenceUnknown: '未知',
    ...
    frameworkUpdateCauses: '更新原因 {causes} · 观测数 {count}',
    frameworkUpdateWindowP95: '框架更新窗口 p95',
    frameworkCheckWindowP95: '框架检查窗口 p95',
