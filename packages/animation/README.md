...
- packaged thin Three/R3F/Pixi/Babylon mappings and renderer target/resource/upload/readback lifecycle evidence; the generic host probe accepts explicit public counters and timer capability, while the optional renderer package supplies sparse asynchronous WebGL 1/2 queries plus explicit single-pass WebGPU timestamp queries; multi-pass and engine-private encoders still need adapters;
