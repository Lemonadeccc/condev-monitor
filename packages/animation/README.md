...
- packaged thin Pixi/Babylon mappings and full renderer resource lifecycle evidence; the generic host probe accepts explicit public counters and timer capability, while the optional renderer package now supplies a thin public-counter Three.js adapter with external-frame capture, the optional React R3F entry passively observes public after-render callbacks without owning the loop, and the renderer package also supplies sparse asynchronous WebGL 1/2 queries, WebGL whole-canvas target-window GPU evidence, explicit single-pass and same-command-buffer multi-pass WebGPU timestamp queries, and a local-only explicit WebGPU scheduled-upload/readback-ready recorder; R3F GPU boundaries, additional resource RUM promotion, per-family composite target evidence, actual GPU transfer duration, inner-object attribution, cross-command-buffer WebGPU, and engine-private encoders still need adapters;
