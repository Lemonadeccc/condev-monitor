...
Caller-attested media stages require a second explicit opt-in. The default RUM v2 report remains frozen at snapshot schema 1; `mediaStages: true` selects snapshot schema 2 and only projects bounded page-level aggregates for `image`, `video`, `canvas`, `webgl`, and `webgpu` attempts created through the Browser-owned recorder:

```ts
const client = init({
    dsn: import.meta.env.VITE_MONITOR_DSN,
    animation: {
        rum: { contractVersion: 2, sampleRate: 0.1, mediaStages: true },
    },
})

const media = client.animation.createMediaSemanticStageRecorder()
const startedAt = performance.now()
const attempt = media.begin('webgl', startedAt)

// Call these only at application-owned semantic boundaries on the same clock.
attempt.decodeReady({ timestampMs: performance.now() })
attempt.uploadReady({ timestampMs: performance.now() })
attempt.firstVisible({ timestampMs: performance.now() })
attempt.end(performance.now())
```

Schema 2 adds 25 closed metrics: first-visible count plus begin-to-decode, decode-to-upload, upload-to-first-visible, and begin-to-first-visible p95 for each supported media kind. It uploads no attempt id, URL, selector, element, resource identity, byte/item count, raw timestamp, or raw sample. `custom` attempts remain local. Missing stages remain unavailable, while rejected, incomplete, or truncated evidence is conservatively partial. These timestamps are caller attestations—not browser decoder, GPU completion, compositor, first-pixel, or source-code causality proof. Enabling the option does not add target-level attribution or an automatic budget/finding.
...
With the browser client, pass the already set-up and running `AnimationIntegration` as the same sink, or use the matching `client.animation.create*` method so the Browser client owns teardown. The Browser handle exposes explicit `createGsapTickerObserver()`, `createLenisScrollObserver()`, `createScrollTriggerObserver()`, `createMotionSemanticCheckpointRecorder()`, and `createMediaSemanticStageRecorder()` convenience methods without auto-discovering host globals or promoting their local snapshots into host/RUM evidence. `createMotionObserverSession()` is the compact host-owned form: it starts any supplied public GSAP ticker, Lenis, and ScrollTrigger observers, connects them to one local checkpoint recorder, and disposes the entire set together. Semantic records still exist only after the application explicitly calls `begin()` and `end()`/`cancel()`; the session never guesses a business animation boundary. A setup error whose host also refuses rollback names `client.animation.retryPendingObserverCleanup()` explicitly; the Browser handle retains those observer references, including after client destruction, until a later call proves removal. All five sink methods return `false` while the collector is not running; lower-level helpers must be installed after the integration has started and disposed during application teardown.
...
Explicit framework scopes now add a separate local-only element sidecar. The SDK associates a real `Element` with a temporary `framework-scope-*` identity and a bounded closed record ring. React accepts only public Profiler render/base-render evidence plus separately measured `commit-attested` work; Vue accepts only development `onRenderTriggered` operations (`get`, `has`, `iterate`); Angular requires an explicit `inputChanged()` signal; Svelte records only its declared tracked-dependency window; Solid records only caller-explicit host script work. These are deliberately different `render`, `update`, `check`, `host-script`, and `commit-attested` kinds. A record can additionally retain only a closed value-free update-cause category (`props`, `state`, `context`, `input`, `signal`, `store`, `dependency`, `effect`, `parent`, `scheduler`, or `unknown`) plus a bounded observed count. React causes are caller-attested for the next Profiler callback, Vue cause counts come from public development render-trigger events, Angular counts come from `ngOnChanges`, Svelte counts only the supplied tracked values, and Solid causes describe only the caller-wrapped synchronous work. Scope ids, labels, reason/source tokens, props, state, dependency values, Elements, and component objects are omitted from RUM v2/v3 inspection and remain in page memory for the local target detail view.
...
Complete host samples, categories, Browser `pageEvidence`, and media semantic stage record details stay local and are absent from the strict `animation_rum` v1 projection. RUM v2 snapshot schema 1 remains the default closed catalog. Explicit `mediaStages: true` selects schema 2 and reconstructs only the 25 closed per-kind media aggregates plus bounded provider evidence; it never spreads the local snapshot. The WebGPU transfer recorder's scheduled bytes/readback-ready latency, media attempt ids, byte/item counts, raw stage timestamps, `custom` attempts, and all resource identity remain local-only. Existing page outcome metrics and aggregate monitor-overhead metrics still reflect the page's real behavior and the actual cost of running enabled probes; local-only does not mean zero-cost. Hidden/offscreen activity in `pageEvidence.workAvoidance` is deliberately named a review candidate: it does not move the production coverage family to measured and never invents a work duration. In the collector snapshot, host evidence can move `renderer`, `resourcesMedia`, or `memoryLifecycle` coverage from `not-instrumented` to **at most** `partial`; it cannot prove complete coverage. Heap/post-GC plateaus, resource-to-visible attribution, cross-command-buffer or engine-owned WebGPU timing, renderer/browser-measured decode/upload/first-visible attribution, and general automatic framework ownership/update attribution remain separate work.
...
- automatic deep Solid computation/why-update attribution, a published partial-compiled Angular directive/compiler integration with Angular 20/21/22 AOT consumer coverage, automatic deep React/Vue/Angular/Svelte component ownership and value-level why-update diffs, independently measured React commit work, and an independent motion package with framework/engine-specific hooks. The framework scopes now keep closed cause categories and bounded counts when public hooks or the caller can prove them, but never retain changed names or values. The core also has public GSAP inventory/ticker, Lenis scroll, ScrollTrigger aggregate/global-event observers, and an explicit `createMotionSemanticCheckpointRecorder()` bridge for caller-owned `begin()` → `end()`/`cancel()` business windows. Those local global events still do not identify a route, component, business interaction, or animation completion by themselves, and the checkpoint bridge does not enter RUM. The Solid package provides anonymous owner attribution plus caller-authorized synchronous host-work self-time, while React provides an anonymous public-Profiler subtree wrapper, Vue provides an explicit public-lifecycle update-window adapter, Angular provides an explicit public component-check adapter plus a decorator-free target binding for app-local directives, and Svelte provides an explicit tracked-dependency `tick()` window without claiming private full-tree ownership or DOM mutation timing;
- automatic Canvas scene discovery and object-level performance causation; the optional renderer package now includes host-owned anonymous Pixi hit-test attribution, explicit Babylon resource-lifecycle checkpoints, and explicit R3F/postprocessing per-pass CPU plus caller-supplied GPU evidence. These adapters do not walk a Pixi stage, prove that one object caused renderer cost, infer a Babylon leak or GPU allocation, or automatically discover every R3F pass. Pixi v8 still exposes no stable public renderer after-render/counter contract equivalent to Babylon's sanctioned instrumentation, so no private Pixi pipeline is treated as supported;
- broader renderer coordination and production promotion; the renderer package now supplies Three/Babylon public-counter adapters, WebGL whole-canvas queries, single-pass and same-command-buffer multi-pass WebGPU timers, the one-`queue.submit([...])` multi-command-buffer timer, and local WebGPU transfer/readback evidence. Resource RUM promotion, per-family composite counts, actual GPU transfer duration, coordination across multiple submits, and engine-private encoders remain separate work;
