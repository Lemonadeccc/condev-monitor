...
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `recordFrameworkStats()` | framework, phase, render/base-render duration, independently measured commit duration, or a separately typed public lifecycle update window |
...
- packaged Angular/Svelte/Solid ownership/update adapters, deep React/Vue component ownership and why-update causes, independently measured React commit work, and broader GSAP/Lenis/ScrollTrigger lifecycle/ticker integration; the React package now provides an anonymous public-Profiler subtree wrapper and the Vue package provides an explicit public-lifecycle update-window adapter, while neither claims private full-tree ownership;
