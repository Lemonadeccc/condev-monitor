...
Complete host samples, categories, and Browser `pageEvidence` stay local and are absent from the strict `animation_rum` v1 projection, even when RUM is enabled. Explicit RUM v2 reconstructs only the small closed aggregate subset present in its versioned catalog—currently including renderer GPU/draw-call/triangle p95—together with provider counts and evidence quality; it never spreads the local snapshot. The WebGPU transfer recorder's scheduled bytes and readback-ready latency remain local-only. The existing page outcome metrics and aggregate monitor-overhead metrics still reflect the page's real behavior and the actual cost of running enabled probes; local-only does not mean zero-cost. Hidden/offscreen activity in `pageEvidence.workAvoidance` is deliberately named a review candidate: it does not move the production coverage family to measured and never invents a work duration. In the collector snapshot, host evidence can move `renderer`, `resourcesMedia`, or `memoryLifecycle` coverage from `not-instrumented` to **at most** `partial`; it cannot prove complete coverage. Heap/post-GC plateaus, resource-to-visible attribution, cross-command-buffer or engine-owned WebGPU timing, decode/upload/first-visible timing, and general automatic framework ownership/update attribution remain separate work.
...
- a packaged Solid ownership/update adapter, a lower-intrusion Angular directive/compiler integration, deep React/Vue/Angular/Svelte component ownership and why-update causes, independently measured React commit work, and broader GSAP/Lenis/ScrollTrigger lifecycle/ticker integration; the React package provides an anonymous public-Profiler subtree wrapper, Vue provides an explicit public-lifecycle update-window adapter, Angular provides an explicit public component-check adapter, and Svelte provides an explicit tracked-dependency `tick()` window without claiming private full-tree ownership or DOM mutation timing;
- packaged thin Three/R3F/Pixi/Babylon mappings and full renderer resource lifecycle evidence; the generic host probe accepts explicit public counters and timer capability, while the optional renderer package supplies sparse asynchronous WebGL 1/2 queries, WebGL whole-canvas target-window GPU evidence, explicit single-pass and same-command-buffer multi-pass WebGPU timestamp queries, and a local-only explicit WebGPU scheduled-upload/readback-ready recorder; production RUM promotion, per-family composite target evidence, actual GPU transfer duration, inner-object attribution, cross-command-buffer WebGPU, and engine-private encoders still need adapters;
