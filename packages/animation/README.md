...
- a packaged thin Pixi mapping, Babylon GPU/resource/target lifecycle evidence, and full renderer resource lifecycle evidence; the generic host probe accepts explicit public counters and timer capability, while the optional renderer package now supplies a thin public-counter Three.js adapter with external-frame capture and a passive Babylon public `SceneInstrumentation` draw-call adapter. The optional React R3F entry passively observes public frame callbacks without owning the loop plus an explicit exclusive-owner sparse WebGL GPU timer mode. The renderer package also supplies sparse asynchronous WebGL 1/2 queries, WebGL whole-canvas target-window GPU evidence, explicit single-pass and same-command-buffer multi-pass WebGPU timestamp queries, and a local-only explicit WebGPU scheduled-upload/readback-ready recorder; additional resource RUM promotion, per-family composite target evidence, actual GPU transfer duration, inner-object attribution, per-pass/post-processing R3F timing, cross-command-buffer WebGPU, and engine-private encoders still need adapters. Pixi v8 currently exposes no stable public renderer after-render/counter contract equivalent to Babylon's sanctioned instrumentation, so no private Pixi pipeline is treated as supported;
- CDP/trace-backed per-frame JS, style, layout, paint, raster, composite, layer, and authored-source attribution;
- renderer/browser-measured media decode/upload/first-visible attribution and autoplay/visibility behavior beyond the implemented RVFC/playback-quality deltas and caller-attested local/schema-2 aggregate stage recorder; resource-to-first-visible attribution, route/unmount resource deltas, heap/lifecycle growth, hidden/offscreen work checks, and representative-device soak runs;
