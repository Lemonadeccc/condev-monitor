...
Explicit framework scopes now add a separate local-only element sidecar. The SDK associates a real `Element` with a temporary `framework-scope-*` identity and a bounded closed record ring. React accepts only public Profiler render/base-render evidence plus separately measured `commit-attested` work; Vue accepts only development `onRenderTriggered` operations (`get`, `has`, `iterate`); Angular requires an explicit `inputChanged()` signal; Svelte records only its declared tracked-dependency window; Solid records only caller-explicit host script work. These are deliberately different `render`, `update`, `check`, `host-script`, and `commit-attested` kinds. Scope ids, labels, reason/source tokens, props, state, dependency values, Elements, and component objects are omitted from RUM v2/v3 inspection and remain in page memory for the local target detail view.
