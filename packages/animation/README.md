...
- packaged thin Three/R3F/Pixi/Babylon mappings and renderer resource/upload/readback lifecycle evidence; the generic host probe accepts explicit public counters and timer capability, while the optional renderer package supplies sparse asynchronous WebGL 1/2 queries, WebGL whole-canvas target-window GPU evidence, and explicit single-pass and same-command-buffer multi-pass WebGPU timestamp queries; inner-object attribution, cross-command-buffer WebGPU, and engine-private encoders still need adapters;
