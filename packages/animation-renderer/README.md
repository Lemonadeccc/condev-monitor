...
Passive capture can poll an already-supplied timer for earlier resolved queries, but it cannot place timer-query boundaries around a render that another engine owns. Therefore the packaged R3F observer intentionally supplies only public Three counters. GPU timing, per-pass attribution, post-processing boundaries, inner-scene hit testing, and renderer-owned resource lifecycle still require explicit renderer integration. An R3F `frameloop="demand"` or `"never"` root produces no passive sample until R3F actually renders, and an after-render callback is not evidence that the browser compositor presented the frame.
