...
type SilencioWindow = Window & { __silencioGroup?: THREE.Group };
...
function updateMaterial(
  ...
  values: Partial<THREE.MeshPhysicalMaterial>,
) {
...
function buildSceneModel(
  ...
  if (id === "marco2") {
    ...
    const mesh = new THREE.Mesh(
      packageMesh.geometry,
      clonePrimaryMaterial(packageMesh),
    );
...
function updateFloatingModel(model: FloatingSceneModel, elapsedTime: number) {
  const { speed, rotationIntensity, floatIntensity, floatingRange } =
    model.config;
  const phase = model.seed + elapsedTime;
  const wave = (phase / 4) * speed;
  const floatY = THREE.MathUtils.mapLinear(
    Math.sin(wave) / 10,
    -0.1,
    0.1,
    floatingRange[0],
...
function addDesktopModelAnimations(
  config: SceneModel,
  model: THREE.Object3D,
  visibleScale: number,
  cleanup: SceneCleanup,
) {
  const pushTween = (tween: gsap.core.Tween) =>
    cleanup.animations.push(tween);
  const pushTrigger = (trigger: ScrollTrigger) =>
    cleanup.animations.push(trigger);
...
function initThreeScene(isMobile: boolean) {
  ...
  sceneModels.forEach((config) => {
    loader.load(
      ...
      (gltf) => {
        ...
        if (!isMobile) {
          addDesktopModelAnimations(
            config,
            builtModel.target,
            builtModel.initialTargetScale,
            cleanup,
          );
        }
  ...
  sceneModels
    .filter(
      (model): model is SceneModel & { id: ProductId } => model.id !== "basket",
    )
    .forEach((model) => {
...
function initDesktop(context: gsap.Context) {
  ...
  ] as const).forEach(([pin, trigger, border, x, end]) => {
    ScrollTrigger.create({
      trigger: pin,
      start: "top 0",
      end: `top ${end}`,
